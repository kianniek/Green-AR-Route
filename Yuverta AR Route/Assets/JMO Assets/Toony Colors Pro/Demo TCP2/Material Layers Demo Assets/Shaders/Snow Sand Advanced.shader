...
Shader "Toony Colors Pro 2/Examples/Material Layers/Snow + Sand Advanced"
{
	Properties
	{
		...
		_NoiseTexture_sand ("Noise Texture", 2D) = "gray" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			) : SV_Target
			{
				...
				float __noise_sand = (  saturate( (1 - TCP2_TEX2D_SAMPLE(_NoiseTexture_sand, _NoiseTexture_sand, input.pack3.xy * _NoiseTexture_sand_ST.xy + _NoiseTexture_sand_ST.zw).a) * imp_101 ) - imp_101 / 2.0 );
				...
				// Material Layers Blending
				 ...
				 __shadowColor = lerp(__shadowColor, __shadowColor_sand, saturate(((__layer_sand + __noise_sand) + (__contrast_sand * 0.5 - 0.5)) / __contrast_sand));
		...
		HLSLINCLUDE
			...
			) : SV_TARGET
			{
				...
				float __layer_sand = saturate(  ( input.pack0.xyz.y * _PositionRange_sand ) + _PositionThreshold_sand );
				float __contrast_sand = ( _contrast_sand );
				float __noise_sand = (  saturate( (1 - TCP2_TEX2D_SAMPLE(_NoiseTexture_sand, _NoiseTexture_sand, input.pack1.xy * _NoiseTexture_sand_ST.xy + _NoiseTexture_sand_ST.zw).a) * imp_103 ) - imp_103 / 2.0 );
